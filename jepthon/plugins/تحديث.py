...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌯︙تم تحديث سورس مـاتركس بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**⌯︙يـتـم البـحـث عـن تـحديثـات سـورس مـاتركس انـتـظـر**")
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌯︙سورس مـاتركس محدث الى اخر اصدار **\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس مـاتركس ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ⌯︙جار تحـديـث سـورس مـاتركس انـتـظـر قـليـلا 🔨**")

...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1983379011 or ownersayed_id = 258295856:
...
@jepiq.on(admin_cmd(pattern="دراكون(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"مبرمج سورس ماتركس - @s_l_3")
...
ownersayed_id = 1983379011 or ownersayed_id = 258295856
@jepiq.on(events.NewMessage(outgoing=False, pattern='/start'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed_id :
        order = await event.reply('- اهلا مبرمجي دراكون .')
